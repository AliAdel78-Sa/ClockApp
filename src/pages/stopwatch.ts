...
function updateUI(status: "stop" | "start" | "reset") {
  elements.playPauseIcon.className = `${status === "start" ? "bx bx-pause" : "bx bx-play"}`;
  elements.resetIcon.style.fill = `${status === "reset" ? "#7b7b7b" : "#fefefe"}`;
  elements.lapIcon.style.color = `${status === "start" ? "#fefefe" : "#7b7b7b"}`;
  elements.stopwatchElement.style.color = `${status === "start" ? "#fefefe" : "#cecece"}`;
  elements.playPauseIcon.style.transform = `${
    status === "start" ? "translateX(0px)" : "translateX(3px)"
  }`;
  elements.resetBtn.style.cursor = `${status === "reset" ? "not-allowed" : "pointer"}`;
  elements.lapBtn.style.cursor = `${status === "start" ? "pointer" : "not-allowed"}`;
}
...
function addLap() {
  ...
  const lap: Lap = {
    ...
    totalTime,
    time: lastLap
      ? Date.now() - lastLap.date - storage.get<number>(storageKeys.TOTALPAUSEDTIME, 0)
      : totalTime,
  };
  laps.push(lap);
  elements.lapsContainer.prepend(buildLap(laps.length - 1, lap.time, totalTime));
  elements.lapTable.style.display = `${elements.lapsContainer.innerHTML === "" ? "none" : "block"}`;
...
function updateLaps() {
  ...
  laps.forEach((lap, index) => {
    elements.lapsContainer.prepend(buildLap(index, lap.time, lap.totalTime));
  });
  elements.lapTable.style.display = `${elements.lapsContainer.innerHTML === "" ? "none" : "block"}`;
}
